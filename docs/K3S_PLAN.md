...
Read-only review dated 2026-09-07. Baseline approved subsequently; see [installation procedure](K3S_INSTALL.md). Installation remains pending.
...
- **Endpoint:** initial `bronco.local` bootstrap failed despite working shell mDNS lookup. Use bronco's reserved IPv4 address in the installed agent config, entered privately at installation. Keep a placeholder in Git. The existing TLS SAN can remain. Ordinary LAN DNS is a later alternative.
- **Host preparation:** curl installation confirmed before staging. Confirm live firewall policy and forwarding. Leave host swap enabled; the earlier suggestion to disable it was withdrawn. Do not enable pod swap as a separate experiment during bootstrap.
...
- **Recovery:** owner accepts no backups initially. Rebuild from scripts/configuration and accept loss of cluster state, secrets, and application data. Backups become separate work when data is worth keeping.
